...
export default class ClinicRegistrations extends React.Component {
 ...
 async Completeprofile()
   ...
   await  AsyncStorage.setItem("RegistrationScreen","CampRegistration,Camp Registeration");
   this.props.navigation.dispatch(CommonActions.reset({index: 1, routes: [{name: 'CampRegistration', },{ name: 'Profiled' },],}))
 ...
 SetMembershipPurchaseData = async ()=>
  ...
  SelectedProgram = (pID, programName) => {
    ...
    if (pID != '') {
      this.setState({ ProgramID: pID, programName: programName });
  ...
  GetActivityCategory = async (programID) => {
    ...
    var Detailurl = 'CustomerCampRegistration/GetCategoryByProgram?ProgramID=' + programID;
    await CallPI('GET', Detailurl, null, null, "", null).then((response) => response.json()).then(responseJson => {
      var category = [];
      console.log(JSON.stringify(responseJson))
      for (let userObject of responseJson.categoryList) {
  ...
  SelectMembership = (values, personName) => {
  ...
  SetActivityCategoryID = async (values) => {
   ...
   // alert(this.state.ActivityCategoryID)
    ...
    if (values != null || values != '') {
      ...
      var Detailurl = 'CustomerCampRegistration/GetCampByCategoryID?id=' + values + '&isGetCamp=true';
      await CallPI('GET', Detailurl, null, null, "", null).then((response) => response.json()).then(responseJson => {
        var category = [];
        for (let userObject of responseJson.categoryList) {
          category.push({ label: userObject.Name, value: userObject.ID });
        ...
        this.setState({
          ActivityCategoryID: values,
          ActivityCategory: cate,
          ClasstimeList: category,
          WeekDaysList: [],
          SelectWeekDays: [],
          WeekList: [],
          DaysList: [],
  ...
  SetActivity = async (values) => {
    ...
    //alert(this.state.ActivityCategoryID+"  -  "+ values)
    if (this.state.Participant == '') {
      //Alert.alert("Alert", "Please select first participant.")
      this.setState({
        Alert_Visibility: true,
    ...
    var purchaseName = ''
    this.setState({ ClasstimeID: values, ClassTime: classtimes, SelectedWeekDays: [], SelectedDays: [], WeekDaysList: [], DaysList: [], WeekList: [] })
    ...
    var weekURL = 'CustomerCampRegistration/GetWeekByCampID?purchaserID=' + this.state.Participant + '&id=' + values + '&membershipID=' + this.state.MembershipID + '&classID=' + this.state.ActivityCategoryID
    var addtoGrid = new Object();
    addtoGrid = {
      CampRegistrationGridItems: JSON.parse(grid)
     
    }
    console.log("url -----    ---   " + weekURL + "Set Activity ==  === " + JSON.stringify(addtoGrid))
   // return false
  ...
  SetWeek = async (values) => {
    ...
    var weekDaysURL = 'CustomerCampRegistration/GetDaysOfTheWeek?campWeekID=' + values+'&isGetWeeks=true';
    await CallPI('POST', weekDaysURL, null, null, "", null).then((response) => response.json()).then(responseJson => {
      ...
      for (let userObject of responseJson.DayOfWeek) {
        WeekList.push({ label: userObject.Name, value: userObject.WeekID });
      }
  ...
  SelectedDays = (SelectedDays) => {
    ...
    console.log("Select Days  = = = =  = " + JSON.stringify(SelectedDays))
  ...
  async AddToGrid() {
    ...
    if (this.state.Participant == '') {
     // Alert.alert("Alert", "please select participant.")
      this.setState({
        Alert_Visibility: true,
        Alert_Title: 'Alert',
    ...
    if (this.state.WeekDaysList.length > 0 && this.state.SelectedWeekDays.length == 0) {
     ...
     // Alert.alert("Warning", "Please select days.")
      this.setState({
        Alert_Visibility: true,
        Alert_Title: 'Alert',
        Alert_Message: "Please select class days.",
        Alert_MessageMode: 'error',
      })
      return false;
    }
    ...
    if (rs == true) {
      ...
      addtoGrid = {
        ...
        CampID: this.state.ClasstimeID,
        CampName: this.state.ClassTime,
        
        membershipStatus: null,
        days: this.state.SelectedWeekDays.length > 0 ? "," + this.state.SelectedWeekDays.map(i => i.value).join(",") : this.state.SelectedDays.length > 0?  "," + this.state.SelectedDays.map(i => i.value).join(","):null,
     ...
     // console.log("REG body   ====" + JSON.stringify(addtoGrid))
     
      var url = 'CustomerCampRegistration/UpdateGrid'
      await CallPI("POST", url, addtoGrid, LoginUserID, fName + " " + lName, cart).then((response) => response.json())
        .then(responseJson => {
          ...
          if (responseJson.Data.lstGrid != null && responseJson.Data.lstGrid.length > 0) {
            AsyncStorage.setItem('CampGrid', JSON.stringify(responseJson.Data.lstGrid));


            this.props.navigation.navigate("CampRegistrationGrid", {
  ...
  CompareDates = async (items, selDates) => {
    ...
    var res = true
    console.log(items.PurchaserID + "==" + this.state.Participant + "&&" + items.ProgramName + "==" + this.state.programName + "&&" + items.Camp + "==" + this.state.ActivityCategory + "&&" + items.CampName + "==" + this.state.ClassTime)
    if (selDates.length > 0) {

      if (items.PurchaserID == this.state.Participant && items.ProgramName == this.state.programName && items.Camp == this.state.ActivityCategory && items.CampName == this.state.ClassTime) {
        var array1 = [];
        array1 = selDates.split(',');
        var array2 = [];
        array2 = items.Days.split(', ');
      console.log("ssssssssssssssssss  dasysss ====" +array1)
      console.log("itemssssssssssssssssssssssss  dasysss ====" +array2)
  ...
  async GotoClinicRegistrationGrid() {
    ...
    if(list=== null  || list.length==0 || list=='' ){
     // Alert.alert("warning","Please click Confirm Selection select first.")
      this.setState({
        Alert_Visibility: true,
        Alert_Title: 'Alert',
        Alert_Message:"Please click Confirm Selection first.",
        Alert_MessageMode: 'error',
      })
    }
    else{
   ...
   CheckedAllWeekDays = async ()=> {
    ...
    //alert(this.state.AllWeekDaysCheckbox)
