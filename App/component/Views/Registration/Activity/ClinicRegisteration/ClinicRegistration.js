...
export default class ClinicRegistrations extends React.Component {
  ...
  await  AsyncStorage.setItem("RegistrationScreen","ClinicRegistration,Clinic Registeration");
  ...
  SelectFamilyMember = (memberID, purchaseName) => {
    ...
    if (memberID != '') 
    {
      ...
      //alert("UPdate "+this.state.ProgramID)
    }
  ...
  GetActivityCategory = async (programID) => {
    ...
    var Detailurl = 'CustomerClinicRegistration/GetClassesByProgram?ProgramID=' + programID + '&isGetClassess=true';
  ...
  SetActivityCategoryID = async (value) => {

  //  alert(value)
    this.setState({ ActivityCategoryID: value })
    ...
    var Detailurl = 'CustomerClinicRegistration/GetClassesTimeByClassID?classID=' + value + '&type=&isGetClassTime=true';
    //alert(Detailurl)
    await CallPI('GET', Detailurl, null, null, "", null).then((response) => response.json()).then(responseJson => {
      ...
      var cate = this.state.ActivityCategoryList.find(data => data.value == value).label;
      this.setState({ ClasstimeList: category })
      this.setState({ ActivityCategory: cate })
      this.setState({
        ClasstimeID:'' , DaysList:[],WeekDaysList:[],
     FreeWeekMessage:'', DiscountMessage:'',WeekList:[]
    })
    })
  ...
  async AddToGrid() {
    ...
    // alert("Test Before  =="+  JSON.stringify(updatedCart))
    var grid = await AsyncStorage.getItem('Grid');
    var rs = true;
    ...
    //  console.log("Grid Bidty" + JSON.stringify(addtoGrid))
      ...
      await CallPI("POST", url, addtoGrid, LoginUserID, fName + " " + lName, cart).then((response) => response.json())
        .then(responseJson => {
          console.log("jass   ===" + JSON.stringify(responseJson))
          ...
          if (responseJson.Data.lstGrid != null && responseJson.Data.lstGrid.length > 0) {
            ...
            this.props.navigation.navigate("ClinicRegistrationGrid", {
  ...
  async componentDidMount() 
  {
   ...
   // this.SelectFamilyMember(26338)
   //   alert("IsFromMembership = = = "+IsFromMembership)
        ...
        <View style={styles.containerWithCard}>
          ...
          <ScrollView >
            <Card containerStyle={styles.PageCard}>
              ...
              {/* <Familys SelectFamilyMember={this.SelectFamilyMember} PersonID={this.state.PersonID} /> */}
