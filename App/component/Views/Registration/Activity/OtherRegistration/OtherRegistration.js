...
FreeWeekMessage:'',
  ...
  CancelAlert = (values) => { this.setState({ Alert_Visibility: values,isPurchaseMembership:false })
  if(this.state.IsPersonProfileCompleted===false)
  {
    this.Completeprofile()
  }
  ...
  await  AsyncStorage.setItem("RegistrationScreen","OtherRegistration,Other Registeration");
  this.props.navigation.dispatch(CommonActions.reset({index: 1, routes: [{name: 'OtherRegistration', },{ name: 'Profiled' },],}))
  ...
  GetActivityCategory = async (programID) => {
    var Detailurl = 'CustomerOtherRegistration/GetClassesByProgram?ProgramID=' + programID + '&isGetClassess=true';
  ...
  SetActivityCategoryID = async (values) => {
    ...
    await CallPI('GET', Detailurl, null, null, "", null).then((response) => response.json()).then(responseJson => {
      ...
      this.setState({ ActivityCategoryID: values, ActivityCategory: cate })
    })
    ...
    await CallPI("POST", url, addtoGrid, LoginUserID, fName + " " + lName, cart).then((response) => response.json())
      .then(responseJson => {
        ...
        if (responseJson.Data.lstGrid != null && responseJson.Data.lstGrid.length > 0) {
         ...
         this.props.navigation.navigate("OtherRegistrationGrid" ,
         {
...
//console.log(items.PurchaserID +"=="+ this.state.Participant +"&&"+ items.Program +"=="+ this.state.programName +"&&"+ items.Class +"=="+ this.state.ActivityCategory +"&&"+ items.ClassTime +"=="+ this.state.ClassTime)
  ...
  var  IsFromMembership=   await AsyncStorage.getItem('IsFromMembership');
     ...
     if(IsFromMembership==="true")
     {
      ...
      var purchaseMembershipfromActivity = await AsyncStorage.getItem('PurchaseMembershipwithActivity');
          ...
          <Card containerStyle={styles.PageCard}>
            ...
            </View>
            {
              this.state.DiscountMessage != '' ?
                ...
                <View style={styles.inputContainer}>
                  ...
                  <Text style={styles.fontFamily}>{this.state.DiscountMessage}</Text>
                </View>
            ...
            }
            {
              this.state.FreeWeekMessage != '' ?
                <View style={styles.inputContainer}>
                  <Text style={styles.fontFamily}>{this.state.FreeWeekMessage}</Text>
                </View>
                : null
            ...
            }
            {
              this.state.WeekDaysList.length > 0 ?
                ...
                <Text style={[styles.fontFamily,styles.font_12]}>Days: </Text>
                    <CCheckBox
                      title={"All"}
            ...
            }
            {
              this.state.DaysList.length > 0 ?
                <View>
                  <View style={[styles.CheckboxListContain]}>
                    <Text style={[styles.fontFamily,styles.font_12]}> Available Days: </Text>
