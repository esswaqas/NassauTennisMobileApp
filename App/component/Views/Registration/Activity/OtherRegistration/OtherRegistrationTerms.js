...
export default class PersonalInfo extends React.Component {
    ...
    async AddToCart(data) {
        try {
            ...
            if(this.state.DocumentsList.length != this.state.DocumentsList.filter(data => data.checked == true).length)
            {
                
                //Alert.alert("Warning","Please read and checked all documents")
                this.setState({
                    Alert_Visibility: true,
                    Alert_Title: 'Alert',
                    Alert_Message: "Please read and checked all documents.",
            ...
            if (!this.state.TermsConditioncheckbox) {
               ...
               // Alert.alert("Warning","Please select term condition to register.")
                this.setState({
                    Alert_Visibility: true,
                    Alert_Title: 'Alert',
                ...
                .then(responseJson => {
                  //  alert(JSON.stringify(responseJson))
                    ...
                    if (responseJson.isSuccess == true) {
                      //  Alert.alert("Success", "Activity has been added to cart.");
                        this.setState({
                            Alert_Visibility: true,
                            Alert_Title: 'Success',
                            Alert_Message: "Activity has been added to cart.",
    ...
    async TermsConditions() {
        ...
        var url = `CustomerLeagueRegistration/RegistrationTermsConditions?type=Other&&isTermsCondition=true`;
    ...
    GetDocumentDetail = async (index) => {
        ...
        var id = array[index]['value'];
         this.setState({ DocumentsList: array ,isLoadingL:true})
        ...
        await CallPI('GET', url, null, null, " ", null).then((response) => response.json())
            .then(responseJson => {
                ...
                this.setState({
                    ...
                    modalVisible: true,
                    IsDocuments:true,isLoadingL:false
                })
            }
            ).catch(error => {
            });
                    ...
                    <View style={[styles.containerWithCard]}>
                        ...
                        <ScrollView>
                            <Card containerStyle={[styles.PageCardHeader]}>
                                ...
                                <View style={styles.inputContainer}>
                                    <Text style={[styles.fontFamily,styles.font_12]}>TERMS OF ACCEPTANCE and SIGNATURE</Text>
                                </View>
                                <View style={styles.inputContainer}>
                                    <FlatList
                                ...
                                <View style={styles.ActivityCheckboxcontainer}>
                                    <View style={{ width: '12%',marginRight:6 }}>
                                        <CustomCheckbox
                                            title={''}
                                    ...
                                    <View style={{ width: '85%' }}>
                                    <Text style={[styles.fontFamily,styles.font_12]}>
